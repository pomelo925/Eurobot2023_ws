...
int main(int argc, char **argv)
{
    ...
    while (keepRunning && survive_simple_wait_for_event(actx, &event) != SurviveSimpleEventType_Shutdown)
    {
        ...
        case SurviveSimpleEventType_PoseUpdateEvent:
        {
            ...
            const char *tracker_serial = "LHR-94135635";
            const char *serial_number = survive_simple_serial_number(pose_event->object);

            if (strcmp(tracker_serial, serial_number))
            {
