...
static ros::Time rostime_from_survivetime(FLT timecode) { return ros::Time().fromSec(timecode + ros_offset); }
...
struct ObjectPublishers
{
    ...
    std::string serial_number() const { return so->serial_number; }
    void publish(const struct SurviveSimpleButtonEvent *button_event)
    {
        ...
        int64_t mask = survive_simple_object_get_button_mask(obj) |
                       (survive_simple_object_get_touch_mask(obj) << SURVIVE_BUTTON_MAX);
    ...
    void publish(int mask, const FLT *accelgyromag, uint32_t timecode, int id)
    {
        ...
        imu.header.stamp = rostime_from_survivetime(
            1e-6 * SurviveSensorActivations_runtime(&so->activations, so->activations.last_imu));
...
int main(int argc, char **argv)
{
    ...
    std::string node_name = "ros_tools_maincc_revise";
