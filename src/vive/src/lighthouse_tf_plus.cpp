...
bool Vive::initializeParams(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
{
    ...
    get_param_ok = nh_local_.param<string>("survive_world_parent_frame", survive_world_parent_frame_, "map");
...
EulerPose Vive::calculate_the_origin()
{
    ...
    // test slerp
    // tf::Quaternion oo_1, oo_2, oo_;
    // oo_1.setRPY(0, 0, 20 * M_PI / 180);
    // oo_2.setRPY(0, 0, 130 * M_PI / 180);
    // oo_ = oo_1.slerp(oo_2, 0.5);
    // tf::Matrix3x3(oo_).getRPY(roll, pitch, yaw);
    // ROS_INFO("Slerp RPY =>%f %f %f ", roll * 180 / M_PI, pitch * 180 / M_PI, yaw * 180 / M_PI);
...
void Vive::timerCallback(const ros::TimerEvent &e)
{
    ...
    else
    {
        ...
        publish_tf(survive_world_frame_, survive_world_parent_frame_, lh_world_origin);

        // publish tf : lighthouse->tracker
        if (!check_lighthouse_tf)
        {
            ROS_WARN_STREAM("[lighthouse_localization] : "
                            << "wait for tf from map to survive_world_origin");
            bool ok_1 = false;
            std::string *error_msg1;
            ok_1 = listener.canTransform(survive_world_frame_, survive_world_parent_frame_, ros::Time(0), error_msg1);
            if (ok_1)
            {
        ...
        {
            tracker_pose = lookup_tf(survive_tracker_frame_, survive_world_parent_frame_);
            // view
            publish_pose(tracker_pose, Survive::Tracker);
        }
    }
}
