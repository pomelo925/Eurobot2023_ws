...
{
    // ROS_INFO("GoalGoalGoalGoalGoal  (%f, %f, %f)", goal_pose_.x_, goal_pose_.y_, goal_pose_.theta_);
                                    ...
                                    double acceleration)
{
    ...
    if (workingMode_ == Mode::TRACKING)
    {
        if (vel_type == Velocity::linear)
        {
            ...
            else
            {
                ...
                if (linear_deceleration_profile_ == "linear")
                {
                    ...
                    // ROS_INFO("max_linear_vel_reached_ = %f", max_linear_vel_reached_);
                    // ROS_INFO("acc = %f", acc);
                    // ROS_INFO("output_vel = %f", output_vel);
                    // if (output_vel < 0.4)
        ...
        if (vel_type == Velocity::angular)
        {
            ...
            output_vel = vel_state_.theta_ + d_vel * _sign;
